...
def get_down_block(
    ...
    #################################################
    ########  PromptPlus specific blocks ############
    #################################################
    elif down_block_type == "PPlusCrossAttnDownBlock2D":
        if cross_attention_dim is None:
            raise ValueError("cross_attention_dim must be specified for CrossAttnDownBlock2D")
        return PPlusCrossAttnDownBlock2D(
            num_layers=num_layers,
            transformer_layers_per_block=transformer_layers_per_block,
            in_channels=in_channels,
            out_channels=out_channels,
...
def get_mid_block(
    ...
    if mid_block_type == "UNetMidBlock2DCrossAttn":
        ...
        return None
    elif mid_block_type == "PPlusUNetMidBlock2DCrossAttn":
        return PPlusUNetMidBlock2DCrossAttn(
            transformer_layers_per_block=transformer_layers_per_block,
            in_channels=in_channels,
            temb_channels=temb_channels,
            dropout=dropout,
...
def get_up_block(
    ...
    elif up_block_type == "PPlusCrossAttnUpBlock2D":
        if cross_attention_dim is None:
            raise ValueError("cross_attention_dim must be specified for CrossAttnUpBlock2D")
        return PPlusCrossAttnUpBlock2D(
            num_layers=num_layers,
            transformer_layers_per_block=transformer_layers_per_block,
            in_channels=in_channels,
            out_channels=out_channels,
...
class PPlusCrossAttnDownBlock2D(CrossAttnDownBlock2D):
    def forward(
        self,
        hidden_states: torch.Tensor,
        temb: Optional[torch.Tensor] = None,
        ...
        encoder_attention_mask: Optional[torch.Tensor] = None,
        ###
        encoder_hidden_states_list: Optional[List[torch.Tensor]] = None,
        ...
        for i, (resnet, attn) in enumerate(blocks):
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                if encoder_hidden_states_list is not None:
                    hidden_states = attn(
                        hidden_states,
                        encoder_hidden_states=encoder_hidden_states_list[select_idx],
                        cross_attention_kwargs=cross_attention_kwargs,
                        attention_mask=attention_mask,
                        encoder_attention_mask=encoder_attention_mask,
                        return_dict=False,
        ...
        return hidden_states, output_states, select_idx
    
class PPlusUNetMidBlock2DCrossAttn(UNetMidBlock2DCrossAttn):
    def forward(
        self,
        hidden_states: torch.Tensor,
        temb: Optional[torch.Tensor] = None,
        ...
        return hidden_states, select_idx
    
class PPlusCrossAttnUpBlock2D(CrossAttnUpBlock2D):
    def forward(
        self,
        hidden_states: torch.Tensor,
        res_hidden_states_tuple: Tuple[torch.Tensor, ...],
